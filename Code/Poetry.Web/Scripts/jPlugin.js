...
; (function ($, window, document, undefined) {
    ...
    $.growl = function (content, options) {
        ...
        var fadeIn = $growl.fadeIn(options.fade_in,
        function (event) {
        ...
        //$growl.bind('close.bs.alert',
        //function (event) {
        //    if (options.onGrowlClose) {
        //        options.onGrowlClose(event);
        //    }
        //});
        //$growl.bind('closed.bs.alert',
        //function (event) {
...
$.fn.InitLogin = function (controll, path) {
    ...
    $btnLogin.click(function () {
        $.post($.SailConfig.ApiRoot + controll + "/login", { "": JSON.stringify($this.GetJson()) }, function (result) {
            if (!result.IsSuccess) { ShowError(result.Msg); return; }
    ...
    tableToExcel: function (table, name) {
        ...
        if (typeof msie !== "undefined" && msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./))      // If Internet Explorer
        {
            if (typeof Blob !== "undefined") {
                ...
                window.navigator.msSaveBlob(new Blob(fullTemplate, { type: "text/html" }), getFileName(e.settings));
            } else throw "ie版本太低";
...
(function (Sail) {
    ...
    $.fn.initForm = function () {
        ...
        form.find("input.select2").each(function () {
            Sail.BindSelect(this);
        });
        ...
        if ($.fn.Province) {
            form.find(".provinceCode").Province();
        }
        form.initEditor();
    ...
    $.fn.ResetForm = function () {
        ...
        if ($.fn.SetProvince) {
            form.find(".provinceCode").each(function () {
                var $this = $(this);
                $this.val("00");
                $this.SetProvince();
            });
    ...
    ///select2赋值
    $.fn.ValSelect2 = function (value) {
        var control = $(this);
        var url = control.data("url");
        var datas = $(document).data(url);
        if (value) {
            if (value.indexOf(",") === 0 && value.lastIndexOf(",") === value.length - 1) {
                if (url && datas) {
                    var selecteds = _.chain(value.split(","))
                        .filter(function (x) { return (null != x); })
                        .value();
                    control.select2("data", _.filter(datas, function (x) { return _.include(selecteds, x["id"]); }));
                }
                else
                    control.select2("data", _.chain(value.split(","))
                        .filter(function (x) { return (null != x); })
                        .map(function (x) { return ({ id: x, name: x }); })
                        .value());
            }
            else {
                if (url && datas) {
                    control.select2("data", _.find(datas, function (x) { return (x["id"] === value); }));
                }
                else {
                    control.select2("data", { id: value, name: value, text: value });
                }
            }
        }
        return control;
...
(function (Sail) {
    ...
    function BindSelect(obj) {
        ...
        var minimumInputLength = $(obj).data("mininput");
        if (!minimumInputLength)
            minimumInputLength = 0;
        var textfield = $(obj).data("text");
        if (!textfield)
            textfield = "name";
        var isMultiple = $(obj).data("multiple") === true;
        var placeholder = $(obj).data("placeholder");
        if (!placeholder)
            placeholder = "请选择…";
        var select = $(obj);
        if (url) {
            //如果找不到缓存的数据，就用ajax方法读取
            var datas = $(document).data(url);
            if (!datas) {
                $.ajax({
                    url: url,
            ...
            $.each(datas, function (indexInArray, valueOfElement) {
                //如果没有id字段，就从指定值赋值一个id字段过来            
                if (field && valueOfElement.id == undefined)
                    valueOfElement.id = valueOfElement[field];
                //生成拼音
                valueOfElement.PyStr = PinYin.ZhToPinyin(valueOfElement[textfield]);
            });
            if ($.fn.select2)
                select.select2({
                    allowClear: true,
                    ...
                    query: function (query) {
                        ...
                        var term = query.term;
                        var datas = $(document).data(url);
                        var src = _.chain(datas);
                        data.results = src.filter(function (item) {
                            if (!term)
                                return true;
                            ...
                            var mod = item.PyStr;
                            var tf1 = mod.indexOf(term) >= 0;
                            var tf2 = text.indexOf(term) >= 0;
                            return (tf1 || tf2);
                        }).value();
                        query.callback(data);
                    },
                    formatResult: function (data) { return data[textfield]; },
...
(function (Sail) {
    ...
    var Binder = (function () {
         ...
         * @returns
         */
        Binder.prototype.GetProperties = function (obj) {
            var props = [];
            for (var p in obj) {
        ...
        Binder.prototype.ToHtml = function (model) {
            var props = this.GetProperties(model);
            for (var index in props) {
                if (props.hasOwnProperty(index)) {
                    var p = props[index];
                    var control = this.item.find("#" + p.Name + ",[name=" + p.Name + "]");
                    if (control.length > 0) {
                        ...
                        if (p.Type == "object") {
                            this.item.find("[data-prop='" + p.Name + "']").each(function (index, obj) {
                                var id = $(obj).prop("id");
                                try {
                                    if (p.Value)
            ...
            this.item.find(".prop:not(input):not(select)").each(function (i, o) {
                var $control = $(o);
                $control.empty();
            ...
            this.item.setToEditor(model);
            this.item.trigger("after.Binder", model);
        };
        ;
        Binder.prototype.SetObject = function (obj) {
            ...
            if ($.fn.SetProvince) {
                this.item.find(".provinceCode").each(function (i, o) {
                    $(o).SetProvince();
                });
            }
        };
...
(function (Sail) {
    //弹窗模板
    Sail.modalTemplates = {
        dialog: "<div class='modal' role='dialog' data-backdrop='static' >" +
        ...
        cancleButton: "<a  class='btn btn-secondary default btnCancel' data-dismiss='modal' aria-hidden='true'></a>",
        okButton: '<button class="btn btn-primary blue btnOk"></button>'
    };
    /**
     * 弹窗插件
     */
...
(function (MsgBox) {
    ...
    function Show(result, successText, act) {
        ...
        if (result.IsSuccess)
            Success(successText ? successText : result.Msg);
        else
            Error(result.Msg);
...
(function (Sail) {
    ...
    var RazorPage = (function () {
        ...
        RazorPage.prototype.isExpand = function () {
        ...
        */
        ...
        RazorPage.prototype.BtnAct = function (data, page, obj) {
            ...
            if (!confirm("确定要" + obj.data("originalTitle") + "么"))
                return;
...
(function (Sail) {
    ...
    var Pagination = (function () {
        function Pagination(set) {
            ...
            if (set.events) {
                $.each(set.events, function (index, event) {
                    if (event.handle && event.act) {
                        _this.Container.ClickOrTouch(event.handle, function () {
            ...
            $(set.footContainer).unbind().ClickOrTouch(".onPage", function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data && data.IsSuccess) {
                        ...
                        _this.Container.Link(settings.bodyTmpl, data.Data.Data);
...
(function (PinYin_1) {
   ...
   */
    ...
    PinYin_1.ZhToPinyin = ZhToPinyin;
    ;
})(PinYin || (PinYin = {}));
